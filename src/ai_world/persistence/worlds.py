...
class WorldRepository:
    ...
    # --- writes ------------------------------------------------------
    def create(self, name: str, width: int, height: int, seed: int) -> World:
        ...
        cur = self._conn.execute(
            "INSERT INTO worlds (name, width, height, seed, tick, created_at, updated_at, grid) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                world.name,
                width,
    ...
    def save(self, world: World) -> None:
        ...
        world.touch()
        self._conn.execute(
            "UPDATE worlds SET name = ?, tick = ?, updated_at = ?, grid = ? WHERE id = ?",
            (
                world.name,
                world.tick,
                _fmt_dt(world.updated_at),
